...
from statistic.stats_bootstrap import (
    yield_analysis,
    validate_cost,
    find_acceptable_result_for_budget,
)

DEFAULT_TOLERANCE = 0.05
winner_tolerance = {
    "hypervolume": 0.005,
    "igd": DEFAULT_TOLERANCE,
    "ahd": DEFAULT_TOLERANCE,
    "spacing": DEFAULT_TOLERANCE,
    "gd": DEFAULT_TOLERANCE,
    "pdi": 0.005,
}
...
def get_weak_winners(scoring, winner, error_rate):
    ...
    return [
        (algo, score)
        for algo, score in scoring
        if algo != winner[0]
        and (min(score, winner[1]) + eps) / (max(score, winner[1]) + eps)
        >= (1 - error_rate)
    ]
            ...
            for problem_name, problem_mod, algorithms in RunResult.each_result(
                ...
                for budget, metric_name in sorted(scoring):
                    ...
                    weak_winners = get_weak_winners(
                        metric_scoring, (algo_win, score), winner_tolerance[metric_name]
                    )
...
def format_result(algo_score, results, count):
    return "{} ({})".format(algo_score[0], algo_score[1]) + (
        "$^*$" if results.count(algo_score[1]) > count else ""
    )
