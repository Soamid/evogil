...
c_pre = [
    [x for x in emoa_points2],
    [1 - math.sqrt(x) - x * math.sin(10 * math.pi * x) for x in emoa_points2],
]
to_remove = [False] + [c_pre[1][i] >= c_pre[1][i - 1] for i in range(1, len(c_pre[1]))]
emoa_c_analytical = [
    [c_pre[0][i] for i in range(len(c_pre[1])) if not to_remove[i]],
    [c_pre[1][i] for i in range(len(c_pre[1])) if not to_remove[i]],
]
fitnesses, dims, name, pareto_front = emoa_fitnesses(
    f1c, gc, hc, 30, "c", emoa_c_analytical
)

pareto_front = trim_dominated(
