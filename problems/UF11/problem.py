...
def z_bis(z):
    return [
        -lambdas[i] * z[i] if z[i] < 0 else lambdas[i] * z[i] if z[i] > 1 else z[i]
...
def g(z_b):
    return sum((z_bi - 0.5) ** 2 for z_bi in z_b)


def base_fit_cos(z_bis, m):
    return (
        functools.reduce(
            operator.mul, [math.cos((z_bis[i] * math.pi) / 2.0) for i in range(m)]
        )
        if m > 1
        else math.cos((z_bis[0] * math.pi) / 2.0)
        if m == 1
        else 1
    )


def base_fit_up(z_b, m):
    return (1 + g(z_b)) * base_fit_cos(z_b, m - 1) * (
        math.sin(z_b[m - 1]) if m < f_dims else 1
    ) + 1


def base_fit_bottom(z_b, m):
    return S(z_b, m - 1) * base_fit_up(z_b, m)
...
def base_fit(x, m):
    Z = z(x)
    z_b = z_bis(Z)
    return base_fit_up(z_b, m) if all_non_negative(Z) else base_fit_bottom(z_b, m)
...
dims = [
    ...
    for xmin, xmax in zip(
        *([[float(x) for x in line.split()] for line in bounds_lines])
    )
]
