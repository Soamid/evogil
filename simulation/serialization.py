...
from simulation.serializer import Serializer, Result, ResultWithMetadata
...
class RunResult:
    ...
    def each_result(results_path=RESULTS_DIR):
        ...
        #         partial(x.generational_distance, pareto=problem_mod.pareto_front)
        #         for x in result_list
        #     ]
        #     yield "igd", "inverse generational distance", [
        #         partial(
        #             x.inverse_generational_distance, pareto=problem_mod.pareto_front
        #         )
        #         for x in result_list
        #     ]
        #     yield "ahd", "average hausdorff distance", [
        #         partial(x.average_hausdorff_distance, pareto=problem_mod.pareto_front)
        #         for x in result_list
        #     ]
        #     yield "epsilon", "epsilon", [
        #         partial(x.epsilon, pareto=problem_mod.pareto_front) for x in result_list
        #     ]
        #     yield "extent", "extent", [
        #         partial(x.extent, pareto=problem_mod.pareto_front) for x in result_list
        #     ]
        #     yield "spacing", "spacing", [
        #         partial(x.spacing, pareto=problem_mod.pareto_front) for x in result_list
        #     ]
        #     yield "ndr", "non domination ratio", [
        #         partial(x.non_domination_ratio, pareto=problem_mod.pareto_front)
        #         for x in result_list
        #     ]
        #     yield "hypervolume", "hypervolume", [
        #         partial(x.hypervolume, pareto=problem_mod.pareto_front)
        #         for x in result_list
        #     ]
        #     yield "pdi", "pareto dominance indicator", [
        #         partial(x.pareto_dominance_indicator, cache=cache) for x in result_list
        #     ]

        def f_algo(problem_path, algo_path, problem_mod):
            by_budget = defaultdict(list)
            for run in RunResult.each_run(
                algo_path.name, problem_path.name, results_path
            ):
            ...
            for budget in sorted(by_budget):
                yield {
                    ...
                    "analysis": metrics_processor.yield_metrics(
                        by_budget[budget], problem_mod
                    ),
                }
    ...
    def store(self, budget, cost, population, population_fitnesses):
        result = Result(population, population_fitnesses, cost=cost)
        store_path = self.serializer.store(result, str(budget))
        self.budgets[budget] = ResultWithMetadata(
            result, store_path, None, self.simulation_case
        )
    ...
    def load(self, budget):
        ...
        file_name = str(budget)
        result = self.serializer.load(file_name)

        res = ResultWithMetadata(
            result,
            self.serializer.get_result_path(file_name),
            None,
            self.simulation_case,
        )
        ...
        with suppress(FileNotFoundError):
            for candidate in sorted(self.serializer.path.iterdir()):
                try:
                    ...
                    res = ResultWithMetadata(
                        population_pickle, candidate, run_no, self.simulation_case
                    )
...
#     def __init__(
#         self, problem, algo, budget, run_no, cost, population, fitnesses, path
#     ):
...
#             metric_path = (
#                 self.path.parent
#                 / "{self.budget}.{metric_name}.pickle".format(**locals())
#             )
#
#             with suppress(FileNotFoundError):
...
#                 self.fitnesses, self.non_dominated_fitnesses, **metric_params
#             )
#
...
#             logger.exception(
#                 "Error: RunResultBudget.budget=%d .cost=%d .path=%s -> metric=%s",
#                 self.budget,
#                 self.cost,
#                 self.path,
#                 metric_name,
...
#             "generational_distance", metric_params={"pareto": pareto}
#         )
#
#     def inverse_generational_distance(self, pareto):
#         return self._get_metric(
#             "inverse_generational_distance", metric_params={"pareto": pareto}
#         )
#
#     def average_hausdorff_distance(self, pareto):
#         return self._get_metric(
#             "average_hausdorff_distance", metric_params={"pareto": pareto}
#         )
#
#     def epsilon(self, pareto):
#         return self._get_metric("epsilon", metric_params={"pareto": pareto})
#
#     def extent(self, pareto):
#         return self._get_metric("extent", metric_params={"pareto": pareto})
#
#     def spacing(self, pareto):
#         return self._get_metric("spacing", metric_params={"pareto": pareto})
#
#     def non_domination_ratio(self, pareto):
#         return self._get_metric(
#             "non_domination_ratio", metric_params={"pareto": pareto}
#         )
#
#     def hypervolume(self, pareto):
#         return self._get_metric("hypervolume", metric_params={"pareto": pareto})
#
...
#                         try:
#                             match = re.fullmatch(
#                                 "(?P<budget>[0-9]+)\.pickle", result_file.name
#                             )
#
#                             budget = int(match.groupdict()["budget"])
...
#                                     algo_path.name, (self.problem, budget, i)
#                                 )
#                             )
...
#             [str(p) for p in problem_path.iterdir()], "*{}_*".format(self.problem)
#         ):
#             os.remove(filename)
