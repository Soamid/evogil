...
class RunResult:
    ...
    def store(self, budget, cost, population, population_fitnesses):
        store_path = self.serializer.store(
            Result(population, population_fitnesses, cost=cost), str(budget)
        )
        self.budgets[budget] = RunResultBudget(
            self.simulation_case.problem_name,
