...
def worker(simulation, simulation_id):
    ...
    try:
        final_driver, problem_mod = factory.prepare(
            simulation.algorithm_name, simulation.problem_name
        )
        ...
        with log_time(
            ...
            def process_results(budget: int):
                ...
                finalpop_fit = [
                    [fit(x) for fit in problem_mod.fitnesses] for x in finalpop
                ]
        ...
        logger.info(
            "Configuartion disabled by %s:%d:%s. simulation case:%s",
            reason[1],
            reason[2],
            reason[3],
            simulation,
