...
from algorithms.base.individual import Individual
...
class IBEAIndividual(Individual):
    def __init__(self, domain, codomain=None, scaled_codomain=None):
        super().__init__(domain, codomain)
        self.scaled_codomain = scaled_codomain


# noinspection PyPep8Naming
class IBEA(DriverGen):
    def __init__(self, population, dims, fitnesses, mutation_variance, crossover_variance, kappa,
                 mating_population_size, mutation_probability):
        ...
        self.population = [IBEAIndividual(ind)
                           for ind in population]

    def population_generator__COST(self):
        cost = 0
        while True:
            ...
            cost += self._scale_objectives__COST()
    ...
    def steps__COST(self, generator, budget=None):
        cost = 0
        for _ in generator:
            cost += self._next_step__COST()
            if budget is not None and cost > self.budget:
                break
        cost += self._scale_objectives__COST()
        self._calculate_fitness()
        self._environmental_selection()
    ...
    def get_indivs_inorder__COST(self):  # COSTGENERATOR
        cost = len(self.population)
        return cost, self.rank(self.population, self.calculate_objectives)

    def finish__COST(self):
        cost = self._scale_objectives__COST()
        self._calculate_fitness()
        self._environmental_selection()
        return cost, [x.v for x in self.individuals]

    # def _next_step__COST(self):
    # cost = self._scale_objectives__COST()
    # self._calculate_fitness()
    #     self._environmental_selection()
    #     self._mating_selection(0.9)
    ...
    def _scale_objectives__COST(self):  # COSTGENERATOR
        def min_max(x):
            return min(x), max(x)

        cost = len(self.individuals)
        measured = [(objective, min_max([objective(ind.v)
                                         for ind in self.individuals]))
                    for objective in self.objectives]
        self.scaled_objectives = [self._scale(objective, min_o, max_o)
    ...
    def _calculate_fitness(self):
        ...
        self.c = max([abs(x)
                      for x in self.indicators.values()])
        self.fitness = {
            x1: sum([
                (-1) * math.exp((-1) * (self.indicators[(x2, x1)] / abs(self.c * self.k + sys.float_info.epsilon)))
    ...
    def _mating_selection(self, p):
        ...
        better = lambda x1, x2: self.fitness[x1] < self.fitness[x2] and x1 or x2 if coin() else self.fitness[x1] > \
                                                                                                self.fitness[
                                                                                                    x2] and x1 or x2
    ...
    def _crossover(self):
        self.mating_individuals = [
            self.crossover(self.mating_individuals[i].v, self.mating_individuals[self.mating_size + i].v) for i in
...
if __name__ == "__main__":
    ...
    individuals = [[random.uniform(-10, 10), random.uniform(-10, 10)] for _ in range(150)]
    kappa = 0.05
    mating_size = 50
    population = IBEA(objectives, dimensions, individuals, kappa, mating_size)
    for i in range(100):
        population.step__COST()
        print(i)
    effect = population.finish__COST()
